...
func TestForwardAsRawChatCompletions_MissingDoneReturnsPartialUsage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, OpenAIStreamPartialUsageReasonMissingTerminal, result.PartialUsageReason)
	require.Greater(t, result.Usage.OutputTokens, 0)
	require.True(t, gjson.GetBytes(upstream.lastBody, "stream_options.include_usage").Bool())
}

func TestForwardAsRawChatCompletions_ClientDisconnectDurationExcludesDrain(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_raw_partial_duration"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
...
func TestForwardAsRawChatCompletions_ClientDisconnectTimeoutReturnsPartialUsage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_raw_timeout_partial"}},
		Body:       upstreamStream,
	}}
	...
	require.NotNil(t, result)
	require.True(t, result.PartialUsage)
	require.Equal(t, OpenAIStreamPartialUsageReasonTimeout, result.PartialUsageReason)
