...
// ProvideOpenAIOAuthService creates OpenAI OAuth service and injects the
// ChatGPT backend-api client factory used for account metadata enrichment.
...
var ProviderSet = wire.NewSet(
	...
	NewAutoConcurrencyUpgradeService,
...
func ProvideRedeemService(
	...
	svc.SetInvitationEligibilityDeps(settingService, usageRepo)
	return svc
}
...
func ProvidePaymentOrderExpiryService(paymentSvc *PaymentService, lockCache LeaderLockCache, db *sql.DB) *PaymentOrderExpiryService {
	svc := NewPaymentOrderExpiryService(paymentSvc, 2*time.Second)
	svc.SetLeaderLock(lockCache, db)
	svc.Start()
	return svc
}
