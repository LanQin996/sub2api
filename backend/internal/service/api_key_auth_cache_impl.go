...
const apiKeyAuthSnapshotVersion = 18 // v18: include contributor rewards, media/search pricing, and reasoning effort policy
...
// Stop releases the subscriber and both process-local Ristretto caches.
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		RouteGroupIDs: snapshot.RouteGroupIDs,
		Key:           key,
		Name:          snapshot.Name,
