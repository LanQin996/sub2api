...
type OpenAIGatewayHandler struct {
	...
	routeGroupCircuitBreaker   *openAIRouteGroupCircuitBreaker
...
func NewOpenAIGatewayHandler(
	...
	return &OpenAIGatewayHandler{
		...
		imageLimiter:             &imageConcurrencyLimiter{},
		routeGroupCircuitBreaker: newOpenAIRouteGroupCircuitBreaker(),
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	seedOpenAIForwardImageIntentHint(c, routeRuntime.channelMapping.Mapped, imageIntent)
	...
	requiredCapability := openAIResponsesRequiredCapability(imageIntent, routeRuntime.requestPlatform)
	...
	for {
		...
		if err != nil {
			...
			reqLog.Warn("openai.account_select_failed",
				zap.Error(openAICompatibleSelectionErrorForLog(err, routeRuntime.requestPlatform)),
				zap.Int("excluded_account_count", len(failedAccountIDs)),
			)
			if len(failedAccountIDs) == 0 {
				if errors.Is(err, service.ErrNoAvailableCompactAccounts) {
					if routeRuntime.switchToNext("selection_no_available_compact") {
						routeRuntime.resetFailoverState(&switchCount, &failedAccountIDs, &sameAccountRetryCount, &lastFailoverErr)
						continue
					}
		...
		accountReleaseFunc, acquired := h.acquireResponsesAccountSlot(c, routeRuntime.currentAPIKey.GroupID, sessionHash, selection, reqStream, &streamStarted, reqLog)
		if !acquired {
			return
		}
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.Forward(c.Request.Context(), c, account, routeRuntime.forwardBody)
		}()
		cyberBlockKeyHTTP := ""
		if service.GetOpsCyberPolicy(c) != nil {
			cyberBlockKeyHTTP = service.CyberSessionBlockKey(apiKey.ID, c, sessionHashBody)
		}
		h.recordCyberPolicyIfMarked(c, routeRuntime.currentAPIKey, account, routeRuntime.currentSubscription, reqModel, err != nil, cyberBlockKeyHTTP, clientRequestedUsageFields(c, routeRuntime.channelMapping, reqModel, ""), service.HashUsageRequestPayload(body))
			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if h.gatewayService.ShouldStopOpenAIOAuth429Failover(account, failoverErr.StatusCode, switchCount, &oauth429FailoverState) {
						if routeRuntime.switchToNext("upstream_429_failover_stopped") {
							routeRuntime.resetFailoverState(&switchCount, &failedAccountIDs, &sameAccountRetryCount, &lastFailoverErr)
							continue
						}
						h.handleFailoverExhausted(c, failoverErr, streamStarted)
		...
		h.submitOpenAIUsageRecordTask(c.Request.Context(), result, func(ctx context.Context) {
			if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
				...
				ChannelUsageFields: clientRequestedUsageFields(c, routeRuntime.channelMapping, reqModel, result.UpstreamModel),
