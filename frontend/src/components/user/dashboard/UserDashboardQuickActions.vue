<template>
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      ...
      <button v-if="showModelMarketplace" @click="router.push('/models')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-sky-100 transition-transform group-hover:scale-105 dark:bg-sky-900/30">
          <Icon name="beaker" size="lg" class="text-sky-600 dark:text-sky-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('dashboard.modelMarketplace') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('dashboard.browseModelsAndPricing') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
...
const { t } = useI18n()
const showModelMarketplace = computed(() => isFeatureFlagEnabled(FeatureFlags.availableChannels))
