      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIOAuth" @click="$emit('openai-quota-reset', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-orange-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" />
              {{ t('admin.accounts.openaiQuotaReset.menu') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'openai-quota-reset', 'create-spark-shadow'])
const { t } = useI18n()
